...
pub(super) fn make_console_writer(
    module_id: &str,
    sink: ConsoleSink,
    level: &'static str,
) -> NativeFunction {
    ...
    unsafe {
        NativeFunction::from_closure(move |_this, args, context| {
            ...
            for (i, arg) in args.iter().enumerate() {
                ...
                let rendered = arg.to_string(context)?.to_std_string_escaped();
...
pub(super) fn control_from_js(
    value: Option<&JsValue>,
    context: &mut Context,
) -> JsResult<ControlValue> {
